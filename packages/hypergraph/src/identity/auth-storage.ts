...
import { deserialize, serialize } from '../messages/serialize.js';
...
export const buildAccountIdStorageKey = () => `graph-framework:${getEnv()}:id`;

export const buildKeysStorageKey = (walletAddress: string) =>
  walletAddress ? `graph-framework:${getEnv()}:keys:${walletAddress}` : '';

export const buildSessionTokenStorageKey = (walletAddress: string) =>
  walletAddress ? `graph-framework:${getEnv()}:session-token:${walletAddress}` : '';
