...
export class ConnectIdentityService extends Context.Tag('ConnectIdentityService')<
  ConnectIdentityService,
  {
    readonly getByAccountAddress: (
      accountAddress: string,
    ) => Effect.Effect<ConnectIdentityResult, ResourceNotFoundError | DatabaseService.DatabaseError>;
...
}).pipe(Layer.effect(ConnectIdentityService), Layer.provide(DatabaseService.layer));
